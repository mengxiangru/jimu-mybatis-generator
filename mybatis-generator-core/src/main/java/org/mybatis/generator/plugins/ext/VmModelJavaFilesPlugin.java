...
public class VmModelJavaFilesPlugin extends PluginAdapter {
    ...
    private TopLevelClass getVmModelClass(IntrospectedTable introspectedTable, String domainObjectName, String domainObjectNameWithLower) {
        ...
        cls.addImportedType("javax.validation.constraints.NotNull");
        cls.addImportedType("javax.validation.constraints.Size");
        cls.addImportedType("org.hibernate.validator.constraints.NotEmpty");
        ...
        for (IntrospectedColumn introspectedColumn : introspectedColumns) {
            ...
            if (plugins.modelFieldGenerated(field, cls, introspectedColumn, introspectedTable, ModelClassType.BASE_RECORD)) {
                if (introspectedColumn.getJdbcType() == Types.VARCHAR) {
                    field.addAnnotation("@NotEmpty(message = \"{Vm" + domainObjectName + "." + field.getName() + ".NotEmpty}\")");
                    field.addAnnotation("@Size(max = " + introspectedColumn.getLength() + ", message = \"{Vm" + domainObjectName + "." + field.getName() + ".Max}\")");
                } else if (introspectedColumn.getJdbcType() == Types.INTEGER || introspectedColumn.getJdbcType() == Types.BIGINT || introspectedColumn.getJdbcType() == Types.SMALLINT) {
                    field.addAnnotation("@NotNull(message = \"{Vm" + domainObjectName + "." + field.getName() + ".NotNull}\")");
                }
                cls.addField(field);
